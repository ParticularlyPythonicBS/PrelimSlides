...
Control and dynamics integration via diffrax Jax library for neural ODEs. *TSit5* algorithm for ODE integration with PID controller with rtol 1e-7 and atol 1e-9. *Stratanovich Milstein* solver for SDE integrartion with PID controller with rtol 1e-7 and atol 1e-9. 1000 epochs Controller training with only 1/5 data for the first half. Implemented in Jax.
