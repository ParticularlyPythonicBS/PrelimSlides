...
Nonlinear dynamics is a powerful paradigm for understanding complex systems. And more and more systems are bocoming more and more complex as we move forward.
...
- The loss needs to be compuationally efficient, and smooth enough to allow for stable gradient based optimization. 
...
:::{.notes}
Devaney's definition of chaos is a general useful defintion for our purposes. Any two of the rules give the other in ℝ.
...
This model was initially proposed to capture synchronization in large weaky-coupled oscillator systems.
...
The time horizon for the integration is changed from the first half and seconf half of training by 5x to allow extrapolation of the controller.
...
The simplest test for this method was a grid of kuraomoto oscillators. The network was trained to control the phase of the oscillators to a desired state.
...
This can be used in the loss function to prescibe the exact dynamics to optimize towards,
...
:::{.notes}
One was thats groups ar integrated into neural networks are as an extension of the traditional convolutation neural network setup. 
Instead of just lifting to some real space which preserves translationals as in traditional CNNs, we lift to a group space which preserves the group symmetries.
:::
...
Then the rectangular reimann  construction is equivalent to:
$$
...
For the stchastic case, the rectangular construction is equivalent to:
$$
