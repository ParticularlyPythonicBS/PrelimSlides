...
::: {.attribution}
HNN by [Greydanus et al.](https://arxiv.org/abs/2006.08656)
:::
...
$$
with noise intensity $\mathbf{D} = (\eta / B) \Loss(\theta) \mathbf{H}(\theta\star)$
...
Control scheme where a model is used for predicting the future behavior of the system over finite time window.
...
Relies on the ergodicity of chaotic systems.
...
## Background: Chaotic Pendulum Array
<div>
...
## Future Work: Recurrence matrices and granular control
![](figures/Result2/recurrence_matrix_lorenz.png){fig-alt="Controlled Pendulum dynamics" fig-align="center" width=70%}

# Dynamics, symmetries, and introspection
## Background: Group theory for Dynamical Systems

Let $\Gamma$ act on $\mathbb{R}^n$ and $f: \mathbb{R}^n \rightarrow \mathbb{R}^m$ . Then $f$ is said to be $\Gamma$-equivariant if $f(\gamma x) = \gamma f(x)$ for all $x \in \mathbb{R}^n$ and $\gamma \in \Gamma$.
...
*Theorem 1*: Let $\sigma \in C(\mathbb{R})$. Then $M(\sigma)$ is dense in $C(\mathbb{R}^n)$, in the topology of uniform convergence on compacta, if and only if $\sigma$ is not a polynomial.
...
i.e. $\forall f\in C(I_n)$ and $\epsilon>0$, there is a sum $G(x)$ such that $\max_{x\in I_n}|f(x)-G(x)|<\epsilon$.
...
:::{.attribution}
Theorem 1 from [Leshno, Pinkus, et. al](https://doi.org/10.1016/S0893-6080(05)80131-5)
Theorem 2 from [Cybenko](https://doi-org.prox.lib.ncsu.edu/10.1007/BF02551274)
:::
